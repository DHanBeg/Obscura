...
//
// Postgres taşınabilirlik: her migration'ın opsiyonel üçüncü alanı (pgSQL)
// postgres'e özgü SQLite-uyumsuz söz dizimi (DATETIME/BLOB/OR IGNORE/
// AUTOINCREMENT) için driver-uyumlu karşılığı taşır — boşsa sql her iki
// motorda da aynen çalışır. m.sql (SQLite metni) HİÇ değiştirilmiyor;
// zaten prod'da uygulanmış tarihsel kayıt, bkz. altta driver seçimi.
// SQLite-özel idempotency toleransı (contains(msg,"duplicate column"...))
// artık driver-bazlı: bkz. IsBenignSchemaError (schema_errors.go).
func runMigrations() error {
	migrations := []struct {
		id    string
		sql   string
		pgSQL string // boşsa sql her iki driver'da da kullanılır; postgres'te farklı söz dizimi gerekiyorsa (DATETIME/BLOB/OR IGNORE/AUTOINCREMENT) buraya postgres-uyumlu karşılığı yazılır
	}{
		...
		INSERT OR IGNORE INTO obs_supply (id, total_supply, circulating, burned, last_updated)
			VALUES (1, '1000000000000000000000000000', '0', '0', datetime('now'));`, `CREATE TABLE IF NOT EXISTS obs_supply (
			id           INTEGER PRIMARY KEY CHECK (id = 1),
			total_supply TEXT NOT NULL,
			circulating  TEXT NOT NULL DEFAULT '0',
			burned       TEXT NOT NULL DEFAULT '0',
		...
		INSERT INTO obs_supply (id, total_supply, circulating, burned, last_updated)
		...
		INSERT OR IGNORE INTO shielded_root (id, root, leaf_count, updated_at)
			VALUES (1, '0', 0, datetime('now'));`, `CREATE TABLE IF NOT EXISTS shielded_root (
			id          INTEGER PRIMARY KEY CHECK (id = 1),
			root        TEXT NOT NULL DEFAULT '0',
			leaf_count  INTEGER NOT NULL DEFAULT 0,
			updated_at  TEXT NOT NULL
		);
		INSERT INTO shielded_root (id, root, leaf_count, updated_at)
			VALUES (1, '0', 0, NOW())
			ON CONFLICT DO NOTHING;`},
		...
		{"086_zk_proof_log_chain", zk.ProofLogChainMigrationSQL, zk.ProofLogChainMigrationSQLPostgres},
	...
	for _, m := range migrations {
		...
		sqlText := m.sql
		if DB.DriverName() == dbi.DriverPostgres && m.pgSQL != "" {
			sqlText = m.pgSQL
		}
