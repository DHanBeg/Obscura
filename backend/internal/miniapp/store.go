...
func InitSchema(db dbi.Querier) error {
	stmts := []string{
...
func ListApps(db dbi.Querier) ([]MiniApp, error) {
	rows, err := db.Query(`
...
func GetApp(db dbi.Querier, id string) (*MiniApp, error) {
	var a MiniApp
	err := db.QueryRow(`
		SELECT id, name, version, developer_did, manifest_json, code_hash, signed_by, status, created_at
...
func InstallApp(db dbi.Querier, appID, userDID string, permissions []string) error {
...
func UninstallApp(db dbi.Querier, appID, userDID string) (bool, error) {
...
func GetUserInstalls(db dbi.Querier, userDID string) ([]MiniAppInstall, error) {
	rows, err := db.Query(`
...
func GetInstall(db dbi.Querier, appID, userDID string) (*MiniAppInstall, error) {
	var i MiniAppInstall
	var grantedJSON string
	err := db.QueryRow(`
		SELECT app_id, user_did, granted_permissions_json, installed_at
...
func IsInstalled(db dbi.Querier, appID, userDID string) bool {
	var count int
...
func UpdateGrantedPermissions(db dbi.Querier, appID, userDID string, permissions []string) error {
	granted, err := json.Marshal(permissions)
	if err != nil {
		return fmt.Errorf("permissions marshal hatasi: %w", err)
	}
...
func CreateSession(db dbi.Querier, appID, userDID string, pid int) (string, error) {
	id := uuid.New().String()
	now := time.Now().UTC().Format(time.RFC3339)
	_, err := db.Exec(`
...
func CloseSession(db dbi.Querier, sessionID string) error {
	now := time.Now().UTC().Format(time.RFC3339)
...
//	    PRIMARY KEY (app_id, permission)
//	)
func MigrateZKPermissionsTable(db dbi.Querier) error {
	_, err := db.Exec(`
		CREATE TABLE IF NOT EXISTS app_zk_permissions (
			app_id     TEXT    NOT NULL,
			permission TEXT    NOT NULL,
...
func RevokeZkPermission(db dbi.Querier, appID, permission string) (bool, error) {
...
func HasZkPermission(db dbi.Querier, appID, permission string) bool {
	var count int
...
func ListZkPermissions(db dbi.Querier, appID string) ([]string, error) {
...
func GetActiveSession(db dbi.Querier, appID, userDID string) (*MiniAppSession, error) {
