...
- **#40 A1 KISMEN ÇÖZÜLDÜ (2026-08-25, kod wiring): "P2P bootstrap discovery BAĞLI DEĞİL" artık geçersiz.** `DiscoverBootstrapPeers` (4 kaynak: env/DNS/ENS/peer_cache) artık `host.go:Start()`'ta fiilen çağrılıyor, sonucu `connectBootstrap`'a besleniyor; `AddPeerCacheMigration` de discovery çağrısı üzerinden tetikleniyor. `SavePeer` başarılı bootstrap bağlantısında yazıyor — peer_cache iki taraflı ölü kod sorunu kapandı. BOOTSTRAP_PEERS env (kaynak 1) geriye uyumlu doğrulandı. Kanıt: `docs/sessions/2026-08-25-a1-libp2p-wiring.md` (canlı smoke log). **Kalan:** `docker-compose.yml` hâlâ `P2P_ENABLED=false`/boş BOOTSTRAP_PEERS ile duruyor (bu oturumun kapsamı dışı — "açmak" A4'te kanıtlanacak); ENS stub hâlâ peer ID'siz hardcoded fallback (her zaman reddedilir); BFT/imza/quorum (A3) ve bootstrap otomasyonu (A2) ayrı iş. Statü: kod-seviyesi çözüldü, prod aktivasyonu launch öncesi ayrı adım.
...
**E1 KAPANDI (2026-08-24, aynı gün, ayrı oturum):** `chat/[id].tsx` gerçek MLS akışına bağlandı (`sendMessage` grup dalı → yeni `mobile/lib/mls/groupChat.ts`'in `sendGroupTextMessage`/`fetchAndDecryptGroupMessages`'ı, 4sn polling — WS bilerek atlandı). CANLI SMOKE PASS (`mls-e1-real-backend.smoke.test.ts`) — gerçek yerel `go run ./cmd/node` + gerçek `/v1/auth/*` (OTP+JWT) + gerçek `/v1/mls/*` relay, Alice↔Bob iki yönlü, ardışık çoklu mesaj. Yerel jest tam suite: 39/39 PASS, 264 test + 1 dürüst skip (backend yoksa). Wiring sırasında 2 gerçek bug bulundu+düzeltildi (mock-relay testlerinin hiç yakalamadığı): (1) `encryptGroupMessage` gönderen-taraf `newState`'i atıyordu → ardışık 2 mesaj aynı ratchet generation/nonce ile şifrelenirdi (AEAD reuse) — fix: `newState` artık dönüyor, her send sonrası `saveGroupState`. (2) `decryptApplicationMessageWire` (ts-mls `processMessage`) state referansını YERİNDE mutasyona uğratıyor — canlı smoke'ta yakalandı (`aes/gcm: invalid ghash tag`, 2. mesaj çözülemedi) — fix: yeni `decryptApplicationMessageWireWithState` + `plaintext-cache.ts` entegrasyonu (mesaj başına bir kez çöz, önbelleğe yaz — chat her poll'da tüm geçmişi tekrar çektiği için forward-secrecy retention penceresi olmadan eski mesajlar kaybolurdu). Kapsam dışı bırakılan (flag): grup için resim/video/dosya/konum/ses gönderimi hâlâ Alert-bloklu, WS real-time push bağlanmadı. 1:1 regresyon yok (x3dh/sealed-sender/ratchet/message-send suite'leri yeşil). Tam kanıt: [[2026-08-24-e1-chat-wiring]] (`docs/sessions/`). **Launch-blocker listesinden düşebilir.**
...
**#30 KAPANDI (2026-08-24/25, iki oturum):** Marketplace UI hem mobile hem web'e bağlandı. Faz 0: `packages/theme` token katmanı + logo altın recolor (`c0ee671`). Faz 1: endpoint envanteri, 1 boşluk bulundu+kapatıldı (GET transaction/dispute + liste sorgusu, kullanıcı onayıyla, `ec806bf`). Faz 2 (mobile, 3 commit: `12b87b8`,`78928b5`,`c2a088b`) + Faz 3 (web, 3 commit: `d93bea6`,`2dcf1c9`,`cc3fa20`) — 5 ekran grubu (liste+detay+satın-al, siparişlerim+dispute, satıcı ilan oluştur) iki platformda. Her iki platformda BAĞIMSIZ canlı smoke (mobile: Jest+gerçek backend, web: Node fetch+gerçek backend) uçtan uca doğrulandı — ilan aç→listede görün→satın al (escrow "held")→release ("released")→dispute aç/gör→**auth guard doğrulandı** (yabancı kullanıcı başkasının transaction'ını göremiyor, 403). Bkz. [[2026-08-25-30-marketplace-web]] (`docs/sessions/`) ve E1'in [[2026-08-24-e1-chat-wiring]] deseniyle aynı format. Kalan/kapsam dışı: satıcı ilan düzenle/sil ekranı yok (backend var), admin dispute-resolve ekranı yok, dispute durumu sadece localStorage/AsyncStorage'da (cihaz değişince kaybolur). **Launch-blocker listesinden düştü.**
