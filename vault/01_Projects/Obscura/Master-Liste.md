...
- [x] **B10.1 — web MLS grup mesajlaşma (mesaj gönder/al + JOIN)** · KAPANDI (2026-08-28, commit `1f5091e`) — keşif (B7 Faz 2, 2026-08-25) bulgusu: `sendMessage` HER ZAMAN `to_id: conv.peer_did` kullanıyordu, `is_group` hiç geçirilmiyordu (grup için peer_did undefined → backend'e boş to_id). Web kendi Signal-ratchet'ini kullanıyordu (`lib/e2ee-session.ts`), MLS hiç yoktu — **cross-platform grup uyumu (mobile MLS ↔ web Signal-ratchet) bu yüzden hiç doğrulanmamıştı** (B7 Faz 0 bulgusu). Faz 0.5 spike (ts-mls Next.js'de bundle+çalışıyor mu, esm/ build zorunluluğu + webpack alias/fallback kanıtlandı) → Faz 1: `frontend/lib/mls/*` (mobile/lib/mls/* alt kümesi — KeyPackage üret/yükle, Welcome'dan JOIN, grup mesajı şifrele/çöz/gönder/al), api.ts'nin yanlış-şekilli ölü mls wrapper'ları gerçek backend sözleşmesiyle düzeltildi. **CROSS-PLATFORM CANLI KANIT** (`mobile/lib/__tests__/mls-b10-web-cross-platform.smoke.test.ts`, gerçek backend): mobile (değişmemiş kod) grup kurdu + web'i (yeni kod) davet etti → web JOIN edip mesaj gönderdi → **mobile gerçek MLS ile çözdü**; mobile ters yönde gönderdi → **web gerçek MLS ile çözdü**. İki yön PASS — **B7'nin "cross-platform grup uyumu hiç doğrulanmadı" açık maddesi KAPANDI, uyum kanıtlandı.**
  - **B10.2 — AÇIK KALEM (kapsam dışı bırakıldı, bilerek):** Web'den grup KURMA (createGroup/addMember) YOK — web şu an SADECE mobil daveti üzerinden JOIN edip mesajlaşabiliyor, sıfırdan grup kuramıyor. welcome-queue (offline alıcı teslimi) ve key rotation da web'e taşınmadı. Kısıt UI'da AÇIK söyleniyor, gizlenmedi: `frontend/app/chats/new-group/page.tsx` "Grubu Oluştur" butonu `WEB_GROUP_CREATE_DISABLED=true` ile devre dışı + banner ("Web'den grup oluşturma şu an desteklenmiyor"), `frontend/app/chats/invites/page.tsx` da aynı kısıtı davet ekranında tekrarlıyor — B7'nin dead-button tuzağına (buton var ama tıklayınca ya yalan söylüyor ya hiçbir şey yapmıyor) düşülmedi.
...
**Şu an (2026-08-28):** 🔴 **BLOK A TAMAMEN KAPANDI** — A1(bulma)+A2(davetli-ağ)+A3(imza)+A4(iki-node kanıt, iyi+kötü senaryo)+A5(liveness) hepsi commit'li, trustless çekirdeği canlı ağda ispatlandı. B7 + B9 + B6 + B5 + B11 + B10.1 de kapandı (B6 sırasında 2 aylık kritik bir yan-bulgu da kapandı: WS auth tamamen kırıktı, real-time push'un tamamı ölüydü; B5 sırasında grup+1:1 ortak bir gizlilik açığı bulundu → B11 açtı; B11 kapanışında da AYRI bir olası kırıklık bulundu — MinIO bucket policy `media/` prefix'ini hiç açmıyor, C10'da netleştirilmeli; B10.1 web'i mobile ile aynı MLS grubuna bağladı, B7'nin cross-platform-doğrulanmadı maddesini kapattı). Sırada: 🟠 BLOK B (ürün tamlığı — B5✅/B6✅/B7✅/B9✅/B10.1✅/B11✅/B8, B10.2 bilerek açık kaldı, A'dan çok daha hafif) ve 🟡 BLOK C (launch hijyeni — MinIO bucket-policy netleştirmesi de C10'a eklendi). Kritik yolda bir sonraki gerçek adım B (B8/B10.2) veya C'den başlar.
